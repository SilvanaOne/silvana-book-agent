...
- The mid-price poller, balance refresh, and the settlement subscription keep running. **Transition caveat:** in-flight V1 DVPs that still need *this agent's* fee or allocation steps cannot complete while the switch is on — without live V1-stream registration the server's fee gate classifies the agent as a regular user, whose off-chain fee payments never satisfy it — and such DVPs cancel at their deadlines (the counterparty's already-paid on-chain fees are lost). Flip the switch during a quiet V1 window.
