...
export function DemoTools({ liquidityseeking, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!liquidityseeking || liquidityseeking.status !== "monitoring") return <div className="muted">Start Liquidity Seeking to enable demo tools.</div>;

  const price = liquidityseeking.currentPrice;
