...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/liquidity-seeking-slippage-bounded/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/liquidity-seeking-slippage-bounded/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/liquidity-seeking-slippage-bounded/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/liquidity-seeking-slippage-bounded/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-liquidity-seeking-slippage-bounded</strong> — adaptive execution of a large parent order that keeps VWAP-vs-mid slippage of every child under a fixed budget. Each cycle the agent snapshots the book, walks the relevant side (buy → offers, sell → bids) accumulating quantity until the running VWAP would breach <span className="mono">max-slippage-bps</span>, then places a child sized to <span className="mono">min(safeQty, maxChunk, remaining)</span> at the last accepted price. The agent waits for that child to clear before probing again.</p>
              <p>The execution engine (<span className="mono">lib/liquidityseeking-engine.ts</span>) mirrors the Rust logic in <span className="mono">crates/agent-liquidity-seeking-slippage-bounded/src/main.rs</span> (see <span className="mono">seek_loop</span> and <span className="mono">max_safe_qty</span>). The book is synthetic: a bid/offer ladder around a GBM-driven mid. No real orders are sent.</p>
