...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/selective-disclosure/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 1000); return () => clearInterval(id); }, [refresh]);

  const start = async (v: FormValues) => {
    const r = await fetch("/api/selective-disclosure/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(v) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const reset = async () => { await fetch("/api/selective-disclosure/reset", { method: "POST" }); refresh(); };
  const reload = async () => { await fetch("/api/selective-disclosure/reload", { method: "POST" }); refresh(); };
  const refilter = async () => { await fetch("/api/selective-disclosure/refilter", { method: "POST" }); refresh(); };
  const verify = async () => { await fetch("/api/selective-disclosure/verify", { method: "POST" }); refresh(); };
  const tamper = async (seq: number) => { await fetch("/api/selective-disclosure/tamper", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ seq }) }); refresh(); };
      ...
      {tab === "docs" && (
        <div className="card">
          ...
          <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
            <p><strong>agent-selective-disclosure</strong> post-processes an <span className="mono">agent-trading-history</span> JSONL log. Filters (kinds, markets, field redaction) are applied and a fresh hash-chained signed log is emitted — you can share it with an auditor without leaking anything else. The output uses the same record schema as trading-history, so <span className="mono">agent-trading-history verify</span> works on either file.</p>
