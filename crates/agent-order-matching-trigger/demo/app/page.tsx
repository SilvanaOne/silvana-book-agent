...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/order-matching-trigger/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/order-matching-trigger/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/order-matching-trigger/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/order-matching-trigger/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-order-matching-trigger</strong> — a trigger sniper. It subscribes to the orderbook depth for one market and watches the top of book. When <span className="mono">best_offer ≤ buy_trigger</span> it places a BID at that offer for <span className="mono">quantity</span> — you are willing to buy at or below the trigger. When <span className="mono">best_bid ≥ sell_trigger</span> it places an OFFER at that bid. Only one open snipe per side at a time — no stacking while a snipe is still live.</p>
              <p>The placement engine (<span className="mono">lib/ordermatching-engine.ts</span>) mirrors the Rust rules in <span className="mono">crates/agent-order-matching-trigger/src/main.rs</span>. The demo synthesizes the top of book from the mid using <span className="mono">bookSpreadBps</span>. Prices follow a GBM random walk — no real orders are sent, and fills are simulated whenever the mid crosses a resting snipe.</p>
