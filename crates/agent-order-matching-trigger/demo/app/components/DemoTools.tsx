...
export function DemoTools({ ordermatching, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!ordermatching || ordermatching.status !== "monitoring") return <div className="muted">Start Order Matching to enable demo tools.</div>;

  const price = ordermatching.currentPrice;
