...
export function DemoTools({ hedging, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!hedging || hedging.status !== "monitoring") return <div className="muted">Start Hedging to enable demo tools.</div>;

  const price = hedging.currentPrice;
