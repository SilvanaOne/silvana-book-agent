...
export function startDca(config: DcaConfig): DcaState {
  ...
  store.events = [{ t: Date.now(), message: `DCA started: ${config.side.toUpperCase()} ${config.amountPerOrder} ${config.market} every ${config.intervalSecs}s at mid ${config.priceOffsetPct >= 0 ? "+" : ""}${config.priceOffsetPct}%` }];
