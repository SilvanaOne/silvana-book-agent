...
Interactive playground for the [`agent-tpsl`](..) Rust agent. Simulate a
...
- `lib/tpsl-engine.ts` — TypeScript port of the trigger rules from
  `crates/agent-tpsl/src/main.rs` (`fn tpsl_loop`). Same trailing-SL
...
If you want to point the same UI at the real `agent-tpsl` binary,
