...
export function DemoTools({ position, onJump, onWalk }: Props) {
  ...
  const preset = (mult: number) => Number((price * mult).toFixed(8));
  ...
  return (
    <div className="stack">
      ...
      <div className="row" style={{ flexWrap: "wrap", gap: 6 }}>
        <button className="ghost" onClick={() => onJump(preset(1.005))}>+0.5%</button>
        <button className="ghost" onClick={() => onJump(preset(1.02))}>+2%</button>
        <button className="ghost" onClick={() => onJump(preset(1.05))}>+5%</button>
        <button className="ghost" onClick={() => onJump(preset(0.995))}>−0.5%</button>
        <button className="ghost" onClick={() => onJump(preset(0.98))}>−2%</button>
        <button className="ghost" onClick={() => onJump(preset(0.95))}>−5%</button>
      </div>
      {tp !== null && (
