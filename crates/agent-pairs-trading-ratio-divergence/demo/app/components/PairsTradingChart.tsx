...
export function PairsTradingChart({ ticks, pairstrading }: Props) {
  if (ticks.length === 0 || !pairstrading) {
    return (
      <div style={{ height: H_TOTAL, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-faint)" }}>
        No data — start Pairs Trading to see leg prices, ratio and signals.
      </div>
    );
  }
  ...
  const target = pairstrading.config.targetRatio;
  const threshold = pairstrading.config.thresholdPct / 100;
  ...
  // Bottom pane bounds (ratio) — include target and bands
  const upperBand = target * (1 + threshold);
  const lowerBand = target * (1 - threshold);
  const botMinRaw = Math.min(...ratios, lowerBand);
  const botMaxRaw = Math.max(...ratios, upperBand);
  const botPad = (botMaxRaw - botMinRaw) * 0.15 || target * 0.02;
  ...
  return (
    <svg viewBox={`0 0 ${W + 30} ${H_TOTAL}`} style={{ width: "100%", height: "auto", maxHeight: 420 }}>
      ...
      {/* Bottom pane: ratio + target + bands */}
      {botTicks.map((v, i) => (
        <g key={`gb${i}`}>
      ...
      <text x={PAD_L} y={H_TOP + GAP + PAD_T - 2} fill="var(--text-faint)" fontSize={10} fontFamily="ui-monospace, monospace">ratio = mid_A / mid_B (target {target.toFixed(4)}, ±{(threshold * 100).toFixed(2)}%)</text>
      ...
      {/* Target line */}
      <line x1={PAD_L} x2={W - PAD_R} y1={yBot(target)} y2={yBot(target)} stroke="var(--accent)" strokeWidth={1.4} opacity={0.9} />
      ...
      <text x={W - PAD_R + 4} y={yBot(target) + 3} fill="var(--accent)" fontSize={10} fontFamily="ui-monospace, monospace">target</text>
    </svg>
  );
}
