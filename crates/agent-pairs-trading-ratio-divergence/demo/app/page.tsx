...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/pairs-trading-ratio-divergence/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/pairs-trading-ratio-divergence/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/pairs-trading-ratio-divergence/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/pairs-trading-ratio-divergence/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-pairs-trading-ratio-divergence</strong> — a minimal stat-arb skeleton. On every poll it fetches the mid on two markets A and B and computes <span className="mono">ratio = mid_A / mid_B</span>. When <span className="mono">|(ratio − target)/target| × 100 &gt; threshold_pct</span> the agent places <em>two</em> limit orders in opposite directions to push the ratio back toward the target:</p>
