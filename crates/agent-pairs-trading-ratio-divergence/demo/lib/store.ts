...
export function startPairsTrading(config: PairsTradingConfig): PairsTradingState {
  ...
  store.events = [
    {
      t: now,
      message: `PT started: A=${config.marketA} B=${config.marketB}, target=${config.targetRatio}, threshold ±${config.thresholdPct}%, qty A=${config.quantityA} B=${config.quantityB}`,
    },
  ];
