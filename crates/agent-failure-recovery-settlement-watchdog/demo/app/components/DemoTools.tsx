...
export function DemoTools({ failurerecovery, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!failurerecovery || failurerecovery.status !== "monitoring") return <div className="muted">Start Failure Recovery to enable demo tools.</div>;

  const price = failurerecovery.currentPrice;
