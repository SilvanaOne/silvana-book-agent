...
export function DemoTools({ portfoliorebalancing, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!portfoliorebalancing || portfoliorebalancing.status !== "rebalancing") return <div className="muted">Start Rebalancing to enable demo tools.</div>;

  const price = portfoliorebalancing.currentPrice;
