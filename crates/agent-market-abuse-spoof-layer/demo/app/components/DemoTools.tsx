...
export function DemoTools({ marketabuse, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!marketabuse || marketabuse.status !== "monitoring") return <div className="muted">Start Market Abuse to enable demo tools.</div>;

  const price = marketabuse.currentPrice;
