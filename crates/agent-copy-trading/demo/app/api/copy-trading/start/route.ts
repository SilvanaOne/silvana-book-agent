...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    const scale = num(body.scale, "scale");
    if (scale <= 0 || scale > 1) throw new Error("scale in (0, 1]");
    ...
    const config: CopyConfig = { leader, follower, scale, markets, marketPool, maxLeaderNotional, maxMirrorNotional, leaderRatePerSec, dryRun };
