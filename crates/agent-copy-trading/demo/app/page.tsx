...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/copy-trading/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 500); return () => clearInterval(id); }, [refresh]);

  const start = async (v: FormValues) => {
    const r = await fetch("/api/copy-trading/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(v) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/copy-trading/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/copy-trading/reset", { method: "POST" }); refresh(); };
