...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/order-expiry-ttl/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/order-expiry-ttl/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/order-expiry-ttl/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/order-expiry-ttl/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-order-expiry-ttl</strong> — periodically sweeps the agent&apos;s own active orders and cancels any that have been resting on the book longer than <span className="mono">max-age-secs</span>. No two-phase signing is needed — just <span className="mono">OrderbookService.GetOrders</span> + <span className="mono">CancelOrder</span>.</p>
