...
export function DemoTools({ orderexpiry, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!orderexpiry || orderexpiry.status !== "monitoring") return <div className="muted">Start Order Expiry to enable demo tools.</div>;

  const price = orderexpiry.currentPrice;
