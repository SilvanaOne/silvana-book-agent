...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/volatility-screening-rolling-stddev/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/volatility-screening-rolling-stddev/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/volatility-screening-rolling-stddev/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/volatility-screening-rolling-stddev/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-volatility-screening-rolling-stddev</strong> — a read-only observability agent that publishes a rolling realized-volatility estimate for a Canton market. On every tick it samples the mid, computes the log-return against the previous sample, and stores it in a bounded window of the last <span className="mono">window</span> values. On every publish cycle it emits <span className="mono">realized_vol_annualized = std(returns) × sqrt(periodsPerYear / pollSecs)</span>.</p>
              <p>The publisher (<span className="mono">lib/volatilityscreening-engine.ts</span>) mirrors the Rust logic in <span className="mono">crates/agent-volatility-screening-rolling-stddev/src/main.rs</span>. No orders are placed. Prices in the demo come from a GBM random walk — use the nudge controls or crank <span className="mono">vol / tick</span> to watch the realized-vol curve react.</p>
