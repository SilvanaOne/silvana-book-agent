...
export function VolatilityScreeningChart({ ticks, volatilityscreening }: Props) {
  if (ticks.length === 0 || !volatilityscreening) {
    return <div style={{ height: H, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-faint)" }}>No data — start Volatility to see mid + realized-vol overlay.</div>;
  }

  const prices = ticks.map((t) => t.price);
  const vols = ticks.map((t) => t.realizedVol).filter((v): v is number => v !== null && Number.isFinite(v));

  const pMin = Math.min(...prices), pMax = Math.max(...prices);
  const pPad = (pMax - pMin) * 0.1 || pMin * 0.02;
  const pLo = pMin - pPad, pHi = pMax + pPad, pRange = pHi - pLo || 1;

  // Vol axis in percent (0..vHi). Default upper bound 10% so early values are visible.
  ...
  const volPath = ticks
    .map((t, i) => t.realizedVol === null ? null : `${i === 0 ? "M" : "L"}${x(t.t).toFixed(1)},${yV(t.realizedVol).toFixed(1)}`)
    .filter((s): s is string => s !== null)
    .join(" ");

  // Return markers: mark ticks whose |log-return| > 2×recent std as "high-vol"
  const std = volatilityscreening.stdReturn;
  const highVolTicks = std !== null && Number.isFinite(std) && std > 0
    ? ticks.filter((t) => t.logReturn !== null && Math.abs(t.logReturn) > 2 * std)
    : [];
  ...
  const lastVol = volatilityscreening.realizedVolAnnualized;
  ...
  return (
    <svg viewBox={`0 0 ${W + 20} ${H}`} style={{ width: "100%", height: "auto", maxHeight: 360 }}>
      ...
      {/* High-vol return markers along the bottom */}
      {highVolTicks.map((t, i) => (
        <line
          key={`hv${i}`}
          x1={x(t.t)} x2={x(t.t)}
          y1={H - PAD_B - 6} y2={H - PAD_B}
          stroke="var(--neg)" strokeWidth={1.5} opacity={0.65}
        />
      ))}

      {/* Realized-vol overlay (orange) — right axis */}
      ...
      <g transform={`translate(${PAD_L + 4}, ${PAD_T + 4})`}>
        ...
        <text x={82} y={9} fill="var(--text-faint)" fontSize={10} fontFamily="ui-monospace, monospace">realized vol (ann.)</text>
      </g>
    </svg>
  );
}
