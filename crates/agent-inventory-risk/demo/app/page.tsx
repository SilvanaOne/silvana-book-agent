...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/inventory-risk/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/inventory-risk/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/inventory-risk/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/inventory-risk/reset", { method: "POST" }); refresh(); };
  const nudge = async (delta: number) => { await fetch("/api/inventory-risk/nudge", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ delta }) }); };
  const autoHedge = async () => { await fetch("/api/inventory-risk/autohedge", { method: "POST" }); refresh(); };
