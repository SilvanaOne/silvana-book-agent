...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/signal-bot-jsonl/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/signal-bot-jsonl/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/signal-bot-jsonl/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/signal-bot-jsonl/reset", { method: "POST" }); refresh(); };
  ...
  const submitInject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const r = await fetch("/api/signal-bot-jsonl/inject", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
      if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
      refresh();
    } catch (ex) {
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-signal-bot-jsonl</strong> — tails a JSONL file of trading signals and submits one limit order per line. Each JSONL line is <span className="mono">{`{"market":"CC-USDC","side":"buy","quantity":"0.5","price":"1.02","ref":"..."}`}</span>. The bot persists its byte offset in a sibling <span className="mono">&lt;signals&gt;.cursor</span> file so it survives restarts without replaying history.</p>
              ...
              <p>The placement engine (<span className="mono">lib/signalbot-engine.ts</span>) mirrors the Rust rules in <span className="mono">crates/agent-signal-bot-jsonl/src/main.rs</span>. Prices are a GBM random walk — no real orders sent.</p>
