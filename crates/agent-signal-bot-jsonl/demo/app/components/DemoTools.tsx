...
export function DemoTools({ signalbot, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!signalbot || signalbot.status !== "monitoring") return <div className="muted">Start Signal Bot to enable demo tools.</div>;

  const price = signalbot.currentPrice;
