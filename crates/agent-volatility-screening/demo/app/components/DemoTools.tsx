...
export function DemoTools({ volatilityscreening, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!volatilityscreening || volatilityscreening.status !== "monitoring") return <div className="muted">Start Volatility to enable demo tools.</div>;

  const price = volatilityscreening.currentPrice;
