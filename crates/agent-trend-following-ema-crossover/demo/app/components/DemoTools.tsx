...
export function DemoTools({ trendfollowing, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!trendfollowing || trendfollowing.status !== "monitoring") return <div className="muted">Start Trend Following to enable demo tools.</div>;

  const price = trendfollowing.currentPrice;
