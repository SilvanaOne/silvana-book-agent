...
export function startAgent(config: CopyConfig): CopyState {
  ...
  s.events = [{ t: now, message: `copy-trading (proportional) started — leader=${config.leader} → follower=${config.follower} effective_scale=${s.agent.effectiveScale.toFixed(4)} (raw=${s.agent.rawScale.toFixed(4)}, max=${config.maxScale})` }];
  startTimer(s);
  return s.agent;
}
