...
export function DemoTools({ yieldrotation, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.5");

  if (!yieldrotation || yieldrotation.status !== "monitoring") return <div className="muted">Start Yield Rotation to enable demo tools.</div>;

  // Reference price = starting price fallback; used only for nudge percentages.
  const price = yieldrotation.config.startingPrice;
  const nudge = (mult: number) => Number((price * mult).toFixed(8));

  return (
    <div className="stack">
      <h3>Nudge base price</h3>
