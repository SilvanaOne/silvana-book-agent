...
export function DemoTools({ pretradecheck, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!pretradecheck || pretradecheck.status !== "monitoring") return <div className="muted">Start Pre-Trade Check to enable demo tools.</div>;

  const price = pretradecheck.currentPrice;
