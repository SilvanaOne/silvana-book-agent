...
export function DemoTools({ riskalert, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!riskalert || riskalert.status !== "monitoring") return <div className="muted">Start Risk Alert to enable demo tools.</div>;

  const price = riskalert.currentPrice;
