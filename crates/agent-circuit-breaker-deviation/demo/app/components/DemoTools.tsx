...
export function DemoTools({ circuitbreaker, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!circuitbreaker) return <div className="muted">Start Circuit Breaker to enable demo tools.</div>;

  const price = circuitbreaker.currentPrice;
