...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/compliance-screening-settlement-rules/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/compliance-screening-settlement-rules/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/compliance-screening-settlement-rules/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/compliance-screening-settlement-rules/reset", { method: "POST" }); refresh(); };
