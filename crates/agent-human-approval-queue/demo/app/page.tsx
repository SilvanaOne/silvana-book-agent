...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/human-approval-queue/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/human-approval-queue/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/human-approval-queue/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/human-approval-queue/reset", { method: "POST" }); refresh(); };
  ...
  const approve = async (id: string) => {
    await fetch("/api/human-approval-queue/approve", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id, by: snap.humanapproval?.config.reviewerName }) });
    refresh();
  };
  const reject = async (id: string) => {
    await fetch("/api/human-approval-queue/reject", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id, by: snap.humanapproval?.config.reviewerName }) });
    refresh();
  };
  const purge = async () => {
    await fetch("/api/human-approval-queue/purge", { method: "POST" });
    refresh();
  };
