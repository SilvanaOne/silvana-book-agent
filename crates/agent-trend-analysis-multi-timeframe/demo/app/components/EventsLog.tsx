...
export function EventsLog({ events }: Props) {
  ...
  return (
    <div className="mono" style={{ fontSize: 12, lineHeight: 1.6, maxHeight: 260, overflow: "auto" }}>
      {items.map((e, i) => {
        // Colour hint: highlight rows by alignment verdict / signal transitions.
        let color = "#b8c5df";
        if (e.message.includes("align=aligned_up") || e.message.includes("→ aligned_up")) color = "#6de6a3";
        else if (e.message.includes("align=aligned_down") || e.message.includes("→ aligned_down")) color = "#ff8dab";
        else if (e.message.includes("align=mixed") || e.message.includes("→ mixed")) color = "#ffd166";
        if (e.message.startsWith("SIGNAL")) color = "#c299ff";
        if (e.message.startsWith("MTF started")) color = "#c299ff";
