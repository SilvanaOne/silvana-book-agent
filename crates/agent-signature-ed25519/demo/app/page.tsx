...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/signature-ed25519/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/signature-ed25519/start", {
      method: "POST",
  ...
  const stop = async () => {
    await fetch("/api/signature-ed25519/stop", { method: "POST" });
    refresh();
  };
  const reset = async () => {
    await fetch("/api/signature-ed25519/reset", { method: "POST" });
    refresh();
  };
  ...
  const doSign = async (message: string) => {
    const r = await fetch("/api/signature-ed25519/sign", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const doVerify = async (message: string, signature: string) => {
    const r = await fetch("/api/signature-ed25519/verify", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const doRotate = async () => {
    const r = await fetch("/api/signature-ed25519/rotate-key", { method: "POST" });
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p>
                <strong>agent-signature-ed25519</strong> — CLI wrapper around Ed25519 signing used off-chain. The real
