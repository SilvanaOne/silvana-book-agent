...
export function DemoTools({ signature, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!signature || signature.status !== "monitoring") return <div className="muted">Start Signature to enable demo tools.</div>;

  const price = signature.currentPrice;
