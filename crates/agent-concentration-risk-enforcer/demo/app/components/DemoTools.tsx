...
export function DemoTools({ concentrationrisk, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!concentrationrisk || concentrationrisk.status !== "monitoring") return <div className="muted">Start Concentration Risk to enable demo tools.</div>;

  const price = concentrationrisk.currentPrice;
