...
export function DemoTools({ notifier, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!notifier || notifier.status !== "monitoring") return <div className="muted">Start Notifier to enable demo tools.</div>;

  const price = notifier.currentPrice;
