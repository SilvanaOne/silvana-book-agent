...
export function startSpotGrid(config: SpotGridConfig): SpotGridState {
  ...
  const deepestBidOffset = (() => {
    let off = config.baseStepPct / 100;
    for (let i = 1; i < config.bidLevels; i++) off *= config.ratio;
    return off;
  })();
  const deepestOfferOffset = (() => {
    let off = config.baseStepPct / 100;
    for (let i = 1; i < config.offerLevels; i++) off *= config.ratio;
    return off;
  })();
  const bidRange = config.midPrice * (1 - deepestBidOffset);
  const offerRange = config.midPrice * (1 + deepestOfferOffset);
  store.events = [
    {
      t: Date.now(),
      message: `Geometric Grid started: ${config.market}, mid ${config.midPrice}, ${config.bidLevels} bids down to ${round8(bidRange)}, ${config.offerLevels} offers up to ${round8(offerRange)}, base_step ${config.baseStepPct}%, ratio ×${config.ratio}, qty/level ${config.qtyPerLevel}`,
    },
  ];
