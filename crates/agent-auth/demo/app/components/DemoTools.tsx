...
export function DemoTools({ auth, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!auth || auth.status !== "monitoring") return <div className="muted">Start Auth to enable demo tools.</div>;

  const price = auth.currentPrice;
