...
export default function Home() {
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/auth/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    setResult(null);
    refresh();
  };
  const stop = async () => { await fetch("/api/auth/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/auth/reset", { method: "POST" }); setResult(null); refresh(); };
  ...
  const genNow = async () => {
    try {
      const r = await fetch("/api/auth/generate", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" });
  ...
  const decodeNow = async () => {
    try {
      const r = await fetch("/api/auth/decode", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" });
      const j = await r.json();
      if (!r.ok) { setResult({ kind: "error", message: j.error ?? `HTTP ${r.status}` }); return; }
  ...
  const verifyNow = async () => {
    try {
      const r = await fetch("/api/auth/verify", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" });
      const j = await r.json();
      if (!r.ok) { setResult({ kind: "error", message: j.error ?? `HTTP ${r.status}` }); return; }
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-auth</strong> — a JWT lifecycle sandbox for the Silvana orderbook. The real CLI (<span className="mono">crates/agent-auth/src/main.rs</span>) mints self-describing Ed25519 tokens (RFC 8037) and offers <span className="mono">generate</span> / <span className="mono">decode</span> / <span className="mono">verify</span> / <span className="mono">user-id</span>. This demo mirrors the four operations against an in-memory clock so you can watch tokens rotate, expire, and be re-issued as time advances.</p>
