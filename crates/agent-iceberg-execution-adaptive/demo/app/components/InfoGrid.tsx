...
export function InfoGrid({ icebergexecution, walk }: Props) {
  ...
  const currentVisible = icebergexecution?.currentVisible ?? null;
  const nextVisible = icebergexecution?.nextVisible ?? null;
  const lastElapsed = icebergexecution?.lastChunkElapsedSecs ?? null;
  const lastReason = icebergexecution?.lastResizeReason ?? null;

  return (
    <div className="info-grid">
      <div className="info-card">
        <div className="info-card-head"><div className="info-card-title">RUNTIME</div></div>
        <div className="kv-row"><span className="k">strategy mode</span><span className="v">iceberg-adaptive</span></div>
      ...
      <div className="info-card">
        ...
        {c ? (
          <>
            ...
            <div className="kv-row"><span className="k">visible range</span><span className="v">{c.minVisible} – {c.maxVisible}</span></div>
            <div className="kv-row"><span className="k">fast / slow</span><span className="v">{c.fastSecs}s / {c.slowSecs}s</span></div>
          ...
          <>
            ...
            <div className="kv-row"><span className="k">visible range</span><span className="v faint">—</span></div>
            <div className="kv-row"><span className="k">fast / slow</span><span className="v faint">—</span></div>
      ...
      <div className="info-card">
        <div className="info-card-head"><div className="info-card-title">ADAPT</div></div>
        <div className="kv-row"><span className="k">current visible</span><span className="v accent">{fmt(currentVisible)}</span></div>
        <div className="kv-row"><span className="k">next visible</span><span className="v accent">{fmt(nextVisible)}</span></div>
        <div className="kv-row"><span className="k">last elapsed</span><span className="v">{lastElapsed !== null ? `${lastElapsed}s` : "—"}</span></div>
        <div className="kv-row"><span className="k">last resize</span><span className={`v ${lastReason === "grow" ? "positive" : lastReason === "shrink" ? "negative" : ""}`}>{lastReason ?? "—"}</span></div>
      </div>
