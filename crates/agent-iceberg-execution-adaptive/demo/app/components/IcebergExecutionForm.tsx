  ...
  return (
    <form onSubmit={submit} className="stack">
      ...
      <div className="grid-2">
        ...
        <div><label>Initial visible</label><input type="number" step="any" value={v.initialVisible} onChange={(e) => upd("initialVisible", e.target.value)} disabled={disabled || busy} /></div>
      </div>
      <div className="grid-2">
        <div><label>Min visible</label><input type="number" step="any" value={v.minVisible} onChange={(e) => upd("minVisible", e.target.value)} disabled={disabled || busy} /></div>
        <div><label>Max visible</label><input type="number" step="any" value={v.maxVisible} onChange={(e) => upd("maxVisible", e.target.value)} disabled={disabled || busy} /></div>
      </div>
      <div className="grid-2">
        <div><label>Fast (s) — grow if elapsed &lt;</label><input type="number" step="1" value={v.fastSecs} onChange={(e) => upd("fastSecs", e.target.value)} disabled={disabled || busy} /></div>
        <div><label>Slow (s) — shrink if elapsed &gt;</label><input type="number" step="1" value={v.slowSecs} onChange={(e) => upd("slowSecs", e.target.value)} disabled={disabled || busy} /></div>
      </div>
      <div className="grid-2">
      ...
      <button type="submit" className="primary" disabled={disabled || busy}>{busy ? "Starting…" : "Start Adaptive Iceberg"}</button>
    </form>
  );
}
