...
export function startIcebergExecution(config: IcebergExecutionConfig): IcebergExecutionState {
  ...
  store.events = [
    {
      t: now,
      message: `Adaptive iceberg started: ${config.market} ${config.side} total=${config.total} initial=${config.initialVisible} range=[${config.minVisible},${config.maxVisible}] fast=${config.fastSecs}s slow=${config.slowSecs}s price=${config.price} maxRuntime=${config.maxRuntimeSecs}s`,
    },
  ];
