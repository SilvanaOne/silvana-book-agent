...
export function DemoTools({ mr, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!mr || mr.status !== "monitoring") return <div className="muted">Start Mean Reversion to enable demo tools.</div>;

  const price = mr.currentPrice;
