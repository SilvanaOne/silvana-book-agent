...
export function DemoTools({ icebergexecution, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!icebergexecution || icebergexecution.status !== "monitoring") return <div className="muted">Start Iceberg to enable demo tools.</div>;

  const price = icebergexecution.currentPrice;
