...
export function DemoTools({ readinesscheck, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!readinesscheck || readinesscheck.status !== "monitoring") return <div className="muted">Start Readiness Check to enable demo tools.</div>;

  const price = readinesscheck.currentPrice;
