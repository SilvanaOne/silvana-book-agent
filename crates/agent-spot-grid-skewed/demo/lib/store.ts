...
export function startSpotGrid(config: SpotGridConfig): SpotGridState {
  ...
  store.events = [
    {
      t: Date.now(),
      message: `Skewed Grid started: ${config.market}, mid ${config.midPrice}, ${config.bidLevels} bids down to ${round8(bidRange)}, ${config.offerLevels} offers up to ${round8(offerRange)}, step ${config.stepPct}%, base_qty ${config.baseQuantity}, target ${config.targetBalance}, alpha ${config.alpha}`,
    },
  ];
