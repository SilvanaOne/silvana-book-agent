  ...
  return (
    <div className="info-grid">
      ...
      <div className="info-card">
        ...
        {c ? (
          <>
            ...
            <div className="kv-row"><span className="k">step</span><span className="v">{c.stepPct}%</span></div>
          </>
        ) : (
          <>
            ...
            <div className="kv-row"><span className="k">step</span><span className="v faint">—</span></div>
          </>
        )}
      </div>
      ...
      <div className="info-card">
        ...
        <div className="kv-row"><span className="k">base qty / level</span><span className="v">{c ? c.baseQuantity : "—"}</span></div>
        <div className="kv-row"><span className="k">balance</span><span className="v">{fmt(spotgrid?.balance)}</span></div>
        <div className="kv-row"><span className="k">skew</span><span className={`v ${(spotgrid?.skew ?? 0) > 0 ? "positive" : (spotgrid?.skew ?? 0) < 0 ? "negative" : ""}`}>{spotgrid ? spotgrid.skew.toFixed(3) : "—"}</span></div>
        <div className="kv-row"><span className="k">bid×</span><span className="v">{spotgrid ? spotgrid.bidMult.toFixed(2) : "—"}</span></div>
        <div className="kv-row"><span className="k">offer×</span><span className="v">{spotgrid ? spotgrid.offerMult.toFixed(2) : "—"}</span></div>
      </div>
