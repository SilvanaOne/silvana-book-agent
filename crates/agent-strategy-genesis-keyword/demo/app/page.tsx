...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/strategy-genesis-keyword/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 1000); return () => clearInterval(id); }, [refresh]);

  const compile = async (spec: string) => {
    const r = await fetch("/api/strategy-genesis-keyword/compile", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ spec }) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const reset = async () => { await fetch("/api/strategy-genesis-keyword/reset", { method: "POST" }); refresh(); };
