...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/dca-portfolio-multi-market/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/dca-portfolio-multi-market/start", {
      method: "POST",
  ...
  const stop = async () => { await fetch("/api/dca-portfolio-multi-market/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/dca-portfolio-multi-market/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p>
                <strong>agent-dca-portfolio-multi-market</strong> — accumulates (or offloads) assets on Canton across
              ...
              <p>
                ...
                loop in <span className="mono">crates/agent-dca-portfolio-multi-market/src/main.rs</span>. Each market runs
