...
#[derive(Parser)]
#[command(name = "agent-dca-portfolio-multi-market")]
#[command(about = "Portfolio DCA agent — accumulate assets across multiple markets in parallel")]
struct Cli {
...
enum Commands {
    ...
    Run {
        ...
        /// Markets to trade on, comma-separated (e.g. "CBTC-CC,CETH-CC")
...
async fn main() -> Result<()> {
    ...
    agent_logic::logging::init_logging(
        ...
        "agent-dca-portfolio-multi-market",
    );
...
async fn run_dca(
    ...
    // Spawn one DCA order placement loop per market. They share a single shutdown signal.
    let dca_shutdown = Arc::new(AtomicBool::new(false));
    for market_id in markets {
        let cfg = config.clone();
        let sd = dca_shutdown.clone();
        tokio::spawn(async move {
            ...
            .await
            {
                error!("DCA loop for {} failed: {:#}", market_id, e);
            }
        });
    }
