...
export function DemoTools({ pnlscreening, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!pnlscreening || pnlscreening.status !== "monitoring") return <div className="muted">Start PnL Screening to enable demo tools.</div>;

  const price = pnlscreening.currentPrice;
