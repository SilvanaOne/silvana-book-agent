...
export async function POST(req: Request): Promise<Response> {
  ...
  if (body.driftPerTick !== undefined) {
    const n = Number(body.driftPerTick);
    if (!Number.isFinite(n)) return NextResponse.json({ error: "driftPerTick invalid" }, { status: 400 });
    patch.driftPerTick = n;
  }
