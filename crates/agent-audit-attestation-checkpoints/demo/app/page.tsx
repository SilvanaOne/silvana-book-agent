...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/audit-attestation-checkpoints/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 500); return () => clearInterval(id); }, [refresh]);

  const start = async (v: FormValues) => {
    const r = await fetch("/api/audit-attestation-checkpoints/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(v) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/audit-attestation-checkpoints/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/audit-attestation-checkpoints/reset", { method: "POST" }); refresh(); };
  const publish = async () => { await fetch("/api/audit-attestation-checkpoints/publish", { method: "POST" }); refresh(); };
      ...
      {tab === "docs" && (
        <div className="card">
          ...
          <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
            <p><strong>agent-audit-attestation-checkpoints</strong> reads the current head of an <span className="mono">agent-trading-history</span> JSONL and emits a signed checkpoint carrying <span className="mono">{`{ ts, head_seq, head_hash, party, signature }`}</span>. Auditors can later use any single checkpoint to prove that earlier records existed at that time — without needing the full log.</p>
