...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/readiness-check/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/readiness-check/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/readiness-check/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/readiness-check/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-readiness-check</strong> — pre-trade gate. Before launching a production trading agent, it verifies the party is provisioned: required per-instrument unlocked balances are present, no stuck/failed settlements above configurable limits, and (optionally) at least one <span className="mono">TransferPreapproval</span> contract exists.</p>
              ...
              <p>The rule logic (<span className="mono">lib/readinesscheck-engine.ts</span>) mirrors the Rust <span className="mono">fn check</span> in <span className="mono">crates/agent-readiness-check/src/main.rs</span>. No real ledger calls are made — this is a UI teaching model of the gate, not a live check.</p>
