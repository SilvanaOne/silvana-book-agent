...
export type Tick = Readonly<{
  t: number;
  price: number;
  sigmaAnnualized: number | null; // fraction (e.g. 0.42 = 42% annualized)
  barHigh: number | null;         // in-flight open bar high at this tick
  barLow: number | null;          // in-flight open bar low at this tick
  barClosed: boolean;             // this tick closed a bar
}>;
...
export function startVolatilityScreening(config: VolatilityScreeningConfig): VolatilityScreeningState {
  ...
  store.ticks = [{
    ...
    barHigh: config.startingPrice,
    barLow: config.startingPrice,
    barClosed: false,
  }];
  store.events = [
    {
      t: Date.now(),
      message: `PARKINSON started: ${config.market}, window=${config.window} bars, period=${config.periodSecs}s, periods/year=${config.periodsPerYear}`,
    },
  ];
...
export function stopVolatilityScreening(): void {
  ...
  if (store.volatilityscreening && store.volatilityscreening.status === "monitoring") {
    ...
    store.events.push({ t: Date.now(), message: "PARKINSON stopped by operator" });
  }
}
...
function tick(store: StoreState) {
  ...
  const now = Date.now();
  const barsBefore = store.volatilityscreening.closedBars.length;
  const { state, events } = step(store.volatilityscreening, price, now);
  store.volatilityscreening = state;
  const barClosed = state.closedBars.length > barsBefore;
  store.ticks.push({
    t: now,
    price,
    sigmaAnnualized: state.sigmaAnnualized,
    barHigh: state.currentBar ? state.currentBar.high : null,
    barLow: state.currentBar ? state.currentBar.low : null,
    barClosed,
  });
