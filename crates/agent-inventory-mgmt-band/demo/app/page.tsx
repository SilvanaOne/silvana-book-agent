...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/inventory-mgmt-band/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/inventory-mgmt-band/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/inventory-mgmt-band/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/inventory-mgmt-band/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-inventory-mgmt-band</strong> — keeps the unlocked balance of a single instrument inside the target band <span className="mono">[target − tolerance, target + tolerance]</span>. On every check interval it inspects the balance and, if it is outside the band, places one <span className="mono">chunk_size</span> order on the configured market to push it back: an OFFER (sell) when balance is above the band, a BID (buy) when it is below. Order price is <span className="mono">mid × (1 + offset/100)</span>.</p>
              <p>The placement engine (<span className="mono">lib/inventorymgmt-engine.ts</span>) mirrors the Rust rules in <span className="mono">crates/agent-inventory-mgmt-band/src/main.rs</span>. Prices are a GBM random walk — no real orders are sent, and fills are simulated whenever the mid crosses a resting order. Only one open rebalance order per side at any time — no stacking.</p>
