...
export function DemoTools({ inventorymgmt, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!inventorymgmt || inventorymgmt.status !== "monitoring") return <div className="muted">Start Inventory Mgmt to enable demo tools.</div>;

  const price = inventorymgmt.currentPrice;
