...
export function DemoTools({ witnesses, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!witnesses || witnesses.status !== "monitoring") return <div className="muted">Start Witnesses to enable demo tools.</div>;

  const price = witnesses.currentPrice;
