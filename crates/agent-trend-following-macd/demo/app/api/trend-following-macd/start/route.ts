...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const config: TrendFollowingConfig = {
      ...
      signalPeriod: num(body.signalPeriod, "signalPeriod"),
    ...
    if (!config.market) return NextResponse.json({ error: "market must be a non-empty string" }, { status: 400 });
    ...
    if (config.signalPeriod < 1) return NextResponse.json({ error: "signalPeriod must be >= 1" }, { status: 400 });
