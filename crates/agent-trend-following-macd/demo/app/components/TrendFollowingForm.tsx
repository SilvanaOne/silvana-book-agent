...
export function TrendFollowingForm({ disabled, onStart }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const sp = Number(v.signalPeriod);
    ...
    if (!Number.isFinite(sp) || sp < 1) {
      setErr("signal period must be >= 1");
      return;
    }
    setBusy(true);
  ...
  return (
    <form onSubmit={submit} className="stack">
      ...
      <div className="grid-2">
        <div><label>Signal EMA period</label><input type="number" step="1" value={v.signalPeriod} onChange={(e) => upd("signalPeriod", e.target.value)} disabled={disabled || busy} /></div>
      ...
      <button type="submit" className="primary" disabled={disabled || busy}>{busy ? "Starting…" : "Start MACD"}</button>
    </form>
  );
}
