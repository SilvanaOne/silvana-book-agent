...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/test-run/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/test-run/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/test-run/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/test-run/reset", { method: "POST" }); refresh(); };
  const trigger = async () => { await fetch("/api/test-run/trigger", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-test-run</strong> is a read-only pre-flight healthcheck. It exercises every gRPC that any trading agent needs (connect + JWT, <span className="mono">GetMarkets</span>, <span className="mono">GetAllActiveOrders</span>, <span className="mono">GetPendingProposals</span>, <span className="mono">GetMarketData</span>, plus <span className="mono">GetPrice</span> and <span className="mono">GetOrderbookDepth</span> when a market is provided) and reports pass/fail with latency per check. Exits non-zero if any check fails — the exit code hook lets you gate deploys.</p>
              <p>This demo simulates each check with a configurable base latency + per-check failure probability so you can watch an all-green run turn red and back. The engine (<span className="mono">lib/testrun-engine.ts</span>) mirrors <span className="mono">crates/agent-test-run/src/main.rs</span>: same check list, same per-check outcome shape.</p>
