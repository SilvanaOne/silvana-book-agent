...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/concentration-risk/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/concentration-risk/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/concentration-risk/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/concentration-risk/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-concentration-risk</strong> — enforcement variant of <span className="mono">agent-risk-exposure</span>. Each cycle it values the portfolio in quote currency (<span className="mono">balance × mid × priceMultiplier</span>), computes each instrument&apos;s share of portfolio and acts on breaches:</p>
              ...
              <p>The engine (<span className="mono">lib/concentrationrisk-engine.ts</span>) mirrors the Rust <span className="mono">sweep()</span> in <span className="mono">crates/agent-concentration-risk/src/main.rs</span>. Balances random-walk each tick so shares actually drift over time — no real orders sent.</p>
              <ul style={{ paddingLeft: 18 }}>
