...
export function TopBar({ active = "dashboard", onChange, live }: Props) {
  ...
  useEffect(() => { const upd = () => { const d = new Date(); const p = (n: number) => n.toString().padStart(2, "0"); setNow(`${p(d.getHours())}:${p(d.getMinutes())}:${p(d.getSeconds())}`); }; upd(); const id = setInterval(upd, 1000); return () => clearInterval(id); }, []);
  const click = (t: Tab) => { setTab(t); onChange?.(t); };
  return (<>
    <div className="topbar">
      <div className="brand"><span className="brand-mark">A</span><span className="brand-title">Audit Anomaly</span><span className="brand-agent">Agent</span><span className="chip">CANTON</span></div>
      <nav className="nav">
    ...
    <div className="statusrow">
      ...
      <span className="status-role">investigator</span>
      <span className="toggle" />
      <span className="hosted">HOSTED ON
        <a href="https://silvana.one" target="_blank" rel="noopener" className="hosted-logo">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/silvana-mark.svg" alt="" width={18} height={18} /><span>silvana</span>
        </a>
      </span>
    </div>
  </>);
