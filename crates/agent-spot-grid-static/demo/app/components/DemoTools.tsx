...
export function DemoTools({ spotgrid, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!spotgrid || spotgrid.status !== "monitoring") return <div className="muted">Start Spot Grid to enable demo tools.</div>;

  const price = spotgrid.currentPrice;
