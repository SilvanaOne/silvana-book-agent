...
export function EventsLog({ events }: Props) {
  ...
  return (
    <div className="mono" style={{ fontSize: 12, lineHeight: 1.6, maxHeight: 260, overflow: "auto" }}>
      {items.map((e, i) => {
        const isTamper = e.message.startsWith("TAMPER");
        const isRecord = e.message.startsWith("RECORD");
        const isGenesis = e.message.includes("GENESIS");
        const color = isTamper ? "#ff5470" : isGenesis ? "#a58cff" : isRecord ? "#6de6a3" : "#b8c5df";
        return (
