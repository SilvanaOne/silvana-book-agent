...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/trading-history-signed-chain/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/trading-history-signed-chain/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/trading-history-signed-chain/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/trading-history-signed-chain/reset", { method: "POST" }); refresh(); };
  ...
  const tamper = async (index: number) => {
    const r = await fetch("/api/trading-history-signed-chain/tamper", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ index }) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-trading-history-signed-chain</strong> — subscribes to a party&apos;s own <span className="mono">order.*</span> + <span className="mono">settlement.*</span> streams and appends every event to a JSONL file as a signed, hash-chained record. Each line carries a <span className="mono">prev_hash</span> pointing at the previous line&apos;s hash and an Ed25519 signature over <span className="mono">(seq, ts, prev_hash, kind, sha256(payload))</span>. Editing any single line breaks the chain from that point forward — that&apos;s exactly what the <em>Tamper record</em> button demonstrates.</p>
              <p>This demo uses a Poisson arrival process to simulate order/settlement events and a lightweight non-cryptographic hash so you can watch the chain form and break in real time. The full agent (see <span className="mono">crates/agent-trading-history-signed-chain/src/main.rs</span>) uses SHA-256 + Ed25519 and can be re-verified offline with <span className="mono">agent-trading-history-signed-chain verify --history-file &lt;path&gt;</span>.</p>
