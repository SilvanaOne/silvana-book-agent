...
export function startInfiniteGrid(config: InfiniteGridConfig): InfiniteGridState {
  ...
  store.events = [
    {
      t: Date.now(),
      message: `Volatility-Scaled Grid started: ${config.market}, ${config.levels} levels/side, qty ${config.quantityPerLevel}, refresh ${config.refreshSecs}s, σ-window ${config.volWindow}, mult ×${config.stepMultiplier}, step bounds [${config.minStepPct}%, ${config.maxStepPct}%]`,
    },
  ];
