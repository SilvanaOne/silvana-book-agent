...
export function DemoTools({ infinitegrid, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!infinitegrid || infinitegrid.status !== "monitoring") return <div className="muted">Start Infinite Grid to enable demo tools.</div>;

  const price = infinitegrid.currentPrice;
