...
pub fn spawn_acs_worker(config: BaseConfig, cache: Arc<AmuletCache>) {
    tokio::spawn(async move {
        ...
        loop {
            if let Err(e) = refresh_amulets(&config, &cache).await {
...
/// Fetch amulets from ledger and refresh cache
async fn refresh_amulets(config: &BaseConfig, cache: &Arc<AmuletCache>) -> anyhow::Result<()> {
