...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/audit-retention-chained/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 1000); return () => clearInterval(id); }, [refresh]);

  const start = async (v: FormValues) => {
    const r = await fetch("/api/audit-retention-chained/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(v) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const reset = async () => { await fetch("/api/audit-retention-chained/reset", { method: "POST" }); refresh(); };
  const rotate = async () => { await fetch("/api/audit-retention-chained/rotate", { method: "POST" }); refresh(); };
  const verify = async () => { await fetch("/api/audit-retention-chained/verify", { method: "POST" }); refresh(); };
  const tamper = async (num: number) => { await fetch("/api/audit-retention-chained/tamper", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ num }) }); refresh(); };
      ...
      {tab === "docs" && (
        <div className="card">
          ...
          <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
            <p><strong>agent-audit-retention-chained</strong> splits a large <span className="mono">agent-trading-history</span> JSONL into per-day (or per-week) slice files. Each slice starts with a signed header that embeds the <span className="mono">prev_slice_hash</span> of the previous slice — the audit chain remains provable across rotations. With <span className="mono">--retention-days N</span>, records older than the window are dropped; the surviving slice headers still chain through.</p>
