...
export function DemoTools({ cashbuffer, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!cashbuffer || cashbuffer.status !== "monitoring") return <div className="muted">Start Cash Buffer to enable demo tools.</div>;

  const price = cashbuffer.currentPrice;
