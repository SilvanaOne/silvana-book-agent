...
export function DemoTools({ portfoliohealth, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!portfoliohealth || portfoliohealth.status !== "monitoring") return <div className="muted">Start Portfolio Health to enable demo tools.</div>;

  const price = portfoliohealth.currentPrice;
