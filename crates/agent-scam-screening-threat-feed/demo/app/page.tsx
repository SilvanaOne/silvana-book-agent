...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/scam-screening-threat-feed/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 500); return () => clearInterval(id); }, [refresh]);

  const start = async (v: FormValues) => {
    const r = await fetch("/api/scam-screening-threat-feed/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(v) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/scam-screening-threat-feed/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/scam-screening-threat-feed/reset", { method: "POST" }); refresh(); };
      ...
      {tab === "docs" && (
        <div className="card">
          ...
          <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
            <p><strong>agent-scam-screening-threat-feed</strong> maintains a set of category buckets (each with a severity <span className="mono">info</span> / <span className="mono">warn</span> / <span className="mono">critical</span> and a list of party ids). Every settlement is checked; matches produce a tagged alert with the top severity. In production, category lists come from files or HTTP feeds that refresh on a schedule; here the demo periodically injects a new pool party into a lower-severity category to mimic that dynamic.</p>
            <p>The engine (<span className="mono">lib/scam-engine.ts</span>) mirrors <span className="mono">crates/agent-scam-screening-threat-feed/src/main.rs</span>.</p>
