...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/yield-rotation/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/yield-rotation/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/yield-rotation/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/yield-rotation/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-yield-rotation</strong> — ranks a basket of markets by a simple yield score and emits a rotation signal whenever the top-ranked market changes. Score is <span className="mono">w_change × change_24h_pct + w_volume × log10(volume_24h) − w_spread × spread_pct</span>, with configurable weights per factor.</p>
              ...
              <p>The scoring engine (<span className="mono">lib/yieldrotation-engine.ts</span>) mirrors the Rust in <span className="mono">crates/agent-yield-rotation/src/main.rs</span>. Per-market metrics are simulated locally so the demo runs without an orderbook connection.</p>
