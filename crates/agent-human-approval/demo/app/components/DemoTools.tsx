...
export function DemoTools({ humanapproval, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!humanapproval || humanapproval.status !== "monitoring") return <div className="muted">Start Human Approval to enable demo tools.</div>;

  const price = humanapproval.currentPrice;
