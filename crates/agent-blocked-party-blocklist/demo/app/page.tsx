...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/blocked-party-blocklist/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/blocked-party-blocklist/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    setBlocklistText(v.blocklist);
    refresh();
  };
  const stop = async () => { await fetch("/api/blocked-party-blocklist/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/blocked-party-blocklist/reset", { method: "POST" }); setBlocklistText(""); refresh(); };
  ...
  const reloadBlocklist = async () => {
    ...
    setReloadBusy(true);
    try {
      const r = await fetch("/api/blocked-party-blocklist/blocklist", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ blocklist: blocklistText }) });
      if (!r.ok) {
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-blocked-party-blocklist</strong> streams settlement events from the Silvana orderbook and, for every <span className="mono">SettlementProposal</span>, checks whether the <span className="mono">buyer</span> or <span className="mono">seller</span> party id appears in an operator-maintained blocklist. If either side matches, the agent emits a <span className="mono">compliance.blocked_party_hit</span> record to stdout / JSONL / HTTP webhook — no orders, no cancels, pure compliance screening.</p>
