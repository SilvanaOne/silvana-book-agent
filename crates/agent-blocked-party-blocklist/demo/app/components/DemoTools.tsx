...
export function DemoTools({ blockedparty, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!blockedparty || blockedparty.status !== "monitoring") return <div className="muted">Start Blocked Party to enable demo tools.</div>;

  const price = blockedparty.currentPrice;
