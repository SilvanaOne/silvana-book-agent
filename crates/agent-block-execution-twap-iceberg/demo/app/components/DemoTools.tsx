...
export function DemoTools({ blockexecution, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!blockexecution || blockexecution.status !== "monitoring") return <div className="muted">Start Block Execution to enable demo tools.</div>;

  const price = blockexecution.currentPrice;
