...
export function DemoTools({ batchorders, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  const running = batchorders && (batchorders.status === "monitoring" || batchorders.status === "submitting");
  if (!batchorders || !running) return <div className="muted">Start Batch Orders to enable demo tools.</div>;

  const price = batchorders.currentPrice;
