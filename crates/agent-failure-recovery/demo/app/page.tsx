...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/failure-recovery/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/failure-recovery/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/failure-recovery/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/failure-recovery/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-failure-recovery</strong> — a watchdog that periodically sweeps this party&apos;s settlement proposals and surfaces problems for an operator to act on. On every sweep it inspects the proposal buffer and flags any <span className="mono">PENDING</span> proposal older than <span className="mono">--max-pending-age-secs</span>, plus any that already transitioned to <span className="mono">FAILED</span>.</p>
