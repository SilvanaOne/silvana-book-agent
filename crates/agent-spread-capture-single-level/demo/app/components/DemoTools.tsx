...
export function DemoTools({ spreadcapture, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!spreadcapture || spreadcapture.status !== "monitoring") return <div className="muted">Start Spread Capture to enable demo tools.</div>;

  const price = spreadcapture.currentPrice;
