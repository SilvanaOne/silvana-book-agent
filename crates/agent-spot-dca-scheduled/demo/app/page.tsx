...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/spot-dca-scheduled/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/spot-dca-scheduled/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/spot-dca-scheduled/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/spot-dca-scheduled/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-spot-dca-scheduled</strong> — Dollar-Cost Averaging on Canton. Every N seconds it places one limit order at mid ± offset, accumulating (or distributing) a base instrument at a controlled cadence.</p>
              <p>The placement engine (<span className="mono">lib/dca-engine.ts</span>) mirrors the Rust rules in <span className="mono">crates/agent-spot-dca-scheduled/src/main.rs</span>. Prices are a GBM random walk — no real orders sent.</p>
