...
export function startPortfolioRebalancing(config: PortfolioRebalancingConfig): PortfolioRebalancingState {
  ...
  store.events = [
    {
      t: Date.now(),
      message: `Rebalancing started: ${config.targets.length} targets, threshold ±${config.thresholdPct}pp, fraction ${config.rebalanceFraction}, cycle ${config.checkIntervalSecs}s`,
    },
  ];
