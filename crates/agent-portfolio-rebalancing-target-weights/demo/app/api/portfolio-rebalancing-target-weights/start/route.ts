...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    const thresholdPct = num(body.thresholdPct, "thresholdPct");
    ...
    if (thresholdPct <= 0) return NextResponse.json({ error: "thresholdPct must be > 0" }, { status: 400 });
    if (rebalanceFraction <= 0 || rebalanceFraction > 1) {
      return NextResponse.json({ error: "rebalanceFraction must be in (0, 1]" }, { status: 400 });
    }
