...
export function DemoTools({ killswitch, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!killswitch || killswitch.status !== "monitoring") return <div className="muted">Start Killswitch to enable demo tools.</div>;

  const price = killswitch.currentPrice;
