...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/block-execution/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/block-execution/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/block-execution/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/block-execution/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-block-execution</strong> — a TWAP × Iceberg hybrid that walks a large parent order through the book without leaking its size. The parent quantity is split into <span className="mono">timeSlices</span> equal-time slices. Within each slice, at most <span className="mono">visible</span> quantity is live on the book at the configured limit price. When a chunk fills, the next chunk is posted until the slice quantity is exhausted or its window expires — any unfilled remainder <span className="mono">carries over</span> into the next slice.</p>
              <p>The placement engine (<span className="mono">lib/blockexecution-engine.ts</span>) mirrors the Rust rules in <span className="mono">crates/agent-block-execution/src/main.rs</span>. Prices are a GBM random walk — no real orders sent. A resting BID is treated as filled when mid ≤ price; a resting OFFER when mid ≥ price.</p>
