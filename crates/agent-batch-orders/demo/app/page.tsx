...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/batch-orders/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/batch-orders/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/batch-orders/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/batch-orders/reset", { method: "POST" }); refresh(); };
  const cancelAll = async () => { await fetch("/api/batch-orders/cancel-all", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              ...
              <p>The engine (<span className="mono">lib/batchorders-engine.ts</span>) is a lightweight port of the state machine in <span className="mono">crates/agent-batch-orders/src/main.rs</span>. Prices are a GBM random walk — no real orders are sent anywhere.</p>
