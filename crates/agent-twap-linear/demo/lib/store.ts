...
const g = globalThis as unknown as { __twapDemoStore?: StoreState };

function getStore(): StoreState {
  if (!g.__twapDemoStore) {
    g.__twapDemoStore = {
      twap: null,
      ticks: [],
      events: [],
...
export function startTwap(config: TwapConfig): TwapState {
  ...
  store.events = [
    {
      t: now,
      message: `TWAP started: ${config.market} ${config.side.toUpperCase()} total=${config.total} slices=${config.slices} duration=${config.durationSecs}s offset=${config.priceOffsetPct}% limit=${lim}`,
    },
  ];
...
export function stopTwap(): void {
  ...
  if (store.twap && store.twap.status === "monitoring") {
    ...
    store.events.push({ t: Date.now(), message: "TWAP stopped by operator" });
  }
}
