...
export function DemoTools({ watchlist, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!watchlist || watchlist.status !== "monitoring") return <div className="muted">Start Watchlist to enable demo tools.</div>;

  const primary = watchlist.snapshots[0];
  const price = primary ? primary.price : 1;
  const nudge = (mult: number) => Number((price * mult).toFixed(8));

  return (
    <div className="stack">
      <h3>Nudge price {primary ? <span className="mono" style={{ fontSize: 11, color: "var(--text-faint)" }}>({primary.market})</span> : null}</h3>
