...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/liquidity-screening/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/liquidity-screening/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/liquidity-screening/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/liquidity-screening/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-liquidity-screening</strong> — polls <span className="mono">GetOrderbookDepth</span> per market and publishes a liquidity snapshot every poll: spread + spread_bps, aggregated bid/offer depth, and a two-sided slippage probe. For a target <span className="mono">probeQty</span> the agent walks the offer side book-level-by-level to compute VWAP for a market buy and the bid side for a market sell, then reports <span className="mono">|vwap − mid| / mid × 10000</span> as slippage in bps. Read-only — no orders, no ledger writes.</p>
              <p>The metric engine (<span className="mono">lib/liquidityscreening-engine.ts</span>) mirrors the Rust logic in <span className="mono">crates/agent-liquidity-screening/src/main.rs</span> (<span className="mono">sample_market</span> + <span className="mono">walk</span>). The synthetic book here breathes with a GBM random-walk mid and level qty jitter, so the depth chart and slippage probes update every tick.</p>
