...
export function DemoTools({ liquidityscreening, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!liquidityscreening || liquidityscreening.status !== "monitoring") return <div className="muted">Start Liquidity to enable demo tools.</div>;

  const price = liquidityscreening.currentPrice;
