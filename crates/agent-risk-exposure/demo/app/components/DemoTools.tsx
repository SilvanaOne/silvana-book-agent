...
export function DemoTools({ riskexposure, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!riskexposure || riskexposure.status !== "monitoring") return <div className="muted">Start Risk Exposure to enable demo tools.</div>;

  const price = riskexposure.currentPrice;
