...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/risk-exposure/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/risk-exposure/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/risk-exposure/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/risk-exposure/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-risk-exposure</strong> — a read-only Tier-3 risk dashboard. Every <span className="mono">snapshotIntervalSecs</span> it walks the party&apos;s balances, prices them at the live mid via each instrument&apos;s <span className="mono">priceMultiplier</span>, and computes portfolio value in quote currency plus the share held by each instrument.</p>
              ...
              <p>The engine (<span className="mono">lib/riskexposure-engine.ts</span>) mirrors <span className="mono">crates/agent-risk-exposure/src/main.rs</span> (<span className="mono">fn snapshot_once</span>). Prices are a GBM random walk — no real ledger calls, and open notional / pending-settlement figures drift slowly to make the dashboard feel live.</p>
