...
export function DemoTools({ targetallocation, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!targetallocation || targetallocation.status !== "monitoring") return <div className="muted">Start Target Allocation to enable demo tools.</div>;

  const price = targetallocation.currentPrice;
