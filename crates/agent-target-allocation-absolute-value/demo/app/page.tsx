...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/target-allocation-absolute-value/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/target-allocation-absolute-value/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/target-allocation-absolute-value/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/target-allocation-absolute-value/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              ...
              <p>The placement engine (<span className="mono">lib/targetallocation-engine.ts</span>) mirrors the Rust rules in <span className="mono">crates/agent-target-allocation-absolute-value/src/main.rs</span>. Prices are a GBM random walk — no real orders sent, and fills are simulated whenever the market mid crosses a resting order.</p>
