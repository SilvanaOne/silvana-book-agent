...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/audit-anomaly-heuristic/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 1000); return () => clearInterval(id); }, [refresh]);

  const start = async (v: FormValues) => {
    const r = await fetch("/api/audit-anomaly-heuristic/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(v) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const reset = async () => { await fetch("/api/audit-anomaly-heuristic/reset", { method: "POST" }); refresh(); };
  const reload = async () => { await fetch("/api/audit-anomaly-heuristic/reload", { method: "POST" }); refresh(); };
  const rescan = async () => { await fetch("/api/audit-anomaly-heuristic/rescan", { method: "POST" }); refresh(); };
