...
export function EventsLog({ events }: Props) {
  ...
  return (
    <div className="mono" style={{ fontSize: 12, lineHeight: 1.6, maxHeight: 260, overflow: "auto" }}>
      {items.map((e, i) => {
        const isFail = e.message.startsWith("SINK-FAIL");
        const isSnap = e.message.includes("depth.snapshot");
        const isPrice = e.message.includes(" price ");
        const color = isFail ? "#ff8dab" : isSnap ? "var(--accent)" : isPrice ? "#6de6a3" : "#b8c5df";
        return (
