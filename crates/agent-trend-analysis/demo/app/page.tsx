...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/trend-analysis/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/trend-analysis/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/trend-analysis/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/trend-analysis/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-trend-analysis</strong> — read-only technical-analysis indicator publisher for Silvana Orderbook on Canton. On every poll it pushes the current mid into a rolling buffer and recomputes:</p>
              ...
              <p>The engine (<span className="mono">lib/trendanalysis-engine.ts</span>) mirrors <span className="mono">crates/agent-trend-analysis/src/main.rs</span>. Every tick emits one <span className="mono">INDICATORS</span> record — the real agent forwards these to stdout, JSONL file or an HTTP webhook. Prices here are a GBM random walk, no ledger writes.</p>
              <ul style={{ paddingLeft: 18 }}>
