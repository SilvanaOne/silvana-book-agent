...
export function DemoTools({ trendanalysis, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!trendanalysis || trendanalysis.status !== "monitoring") return <div className="muted">Start Trend Analysis to enable demo tools.</div>;

  const price = trendanalysis.currentPrice;
