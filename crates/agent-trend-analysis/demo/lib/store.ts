import { initState, step, type TrendAnalysisConfig, type TrendAnalysisState } from "./trendanalysis-engine";
  ...
  store.events = [
    {
      t: Date.now(),
      message:
        `TA started: ${config.market}, window=${config.window}, RSI(${config.rsiPeriod}), ` +
        `BB k=${config.bollingerK}, MACD(${config.macdFast}/${config.macdSlow}/${config.macdSignal})`,
    },
  ];
...
export function stopTrendAnalysis(): void {
  ...
  if (store.trendanalysis && store.trendanalysis.status === "monitoring") {
    ...
    store.events.push({ t: Date.now(), message: "TA stopped by operator" });
  }
}
...
function tick(store: StoreState) {
  ...
  store.ticks.push({
    ...
    macdSignalLine: state.macdSignalLine,
