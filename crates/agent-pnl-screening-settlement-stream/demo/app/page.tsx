...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/pnl-screening-settlement-stream/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/pnl-screening-settlement-stream/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/pnl-screening-settlement-stream/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/pnl-screening-settlement-stream/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-pnl-screening-settlement-stream</strong> — read-only observer of the party&apos;s settlement stream on Canton. For every settled fill it updates the per-market <span className="mono">position</span> and <span className="mono">weighted-avg cost basis</span>, accrues <span className="mono">realized PnL</span> when a sell reduces a long position (mirror for shorts), and every <span className="mono">--snapshot-secs</span> recomputes <span className="mono">unrealized PnL = position × (mid − cost)</span> using the live mid price. No orders, no ledger writes.</p>
