...
export function DemoTools({ statemonitor, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!statemonitor || statemonitor.status !== "monitoring") return <div className="muted">Start State Monitor to enable demo tools.</div>;

  const price = statemonitor.currentPrice;
  const nudge = (mult: number) => Number((price * mult).toFixed(8));

  return (
    <div className="stack">
      <h3>Nudge underlying mid</h3>
