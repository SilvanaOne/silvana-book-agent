...
export function PriceChart({ ticks, position }: Props) {
  ...
  const lines: number[] = [position.tpLeg.price, position.slLeg.price, position.config.entryPrice];
  ...
  const tpActive = position.tpLeg.status === "resting" || position.tpLeg.status === "filled";
  const slActive = position.slLeg.status === "resting" || position.slLeg.status === "filled";
  ...
  return (
    <svg viewBox={`0 0 ${W + 80} ${H}`} style={{ width: "100%", height: "auto", maxHeight: 340 }}>
      ...
      {/* TP line */}
      {tpActive && <HLine v={position.tpLeg.price} color="#6de6a3" label={`TP ${position.tpLeg.price.toFixed(digits)} (${position.tpLeg.status})`} />}
      {/* SL line */}
      {slActive && <HLine v={position.slLeg.price} color="#ff8dab" label={`SL ${position.slLeg.price.toFixed(digits)} (${position.slLeg.status})`} />}
