...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/tpsl-bracket/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    const body = { market: v.market, side: v.side, quantity: v.quantity, entryPrice: v.entryPrice,
      tp: v.tp, sl: v.sl };
    const r = await fetch("/api/tpsl-bracket/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/tpsl-bracket/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/tpsl-bracket/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-tpsl-bracket</strong> watches a live orderbook price and fires an exit when Take-Profit or Stop-Loss is hit. Trailing stops ratchet with peak (long) / trough (short).</p>
              <p>The trigger engine (<span className="mono">lib/tpsl-engine.ts</span>) mirrors the Rust rules in <span className="mono">crates/agent-tpsl-bracket/src/main.rs</span>. Prices are a GBM random walk — no real orders sent.</p>
            </div>
          </div>
        )}
      </div>
