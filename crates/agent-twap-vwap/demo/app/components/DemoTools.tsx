...
export function DemoTools({ twap, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!twap || twap.status !== "monitoring") return <div className="muted">Start TWAP to enable demo tools.</div>;

  const price = twap.currentPrice;
