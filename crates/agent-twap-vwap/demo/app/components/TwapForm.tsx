...
export type FormValues = {
  ...
  volumeCurvePreset: string;
  volumeCurveCustom: string;
};
...
const DEFAULTS: FormValues = {
  ...
  limitPrice: "",
  startingPrice: "0.15",
  volumeCurvePreset: "u-shaped",
  volumeCurveCustom: "",
};

const PRESETS: readonly { value: string; label: string }[] = [
  { value: "u-shaped", label: "U-shaped (heavy open/close)" },
  { value: "linear", label: "Linear (uniform)" },
  { value: "front-loaded", label: "Front-loaded (decreasing)" },
...
export function TwapForm({ disabled, onStart }: Props) {
  ...
  const customMode = v.volumeCurvePreset === "custom";
  ...
  return (
    <form onSubmit={submit} className="stack">
      ...
      <div className="grid-2">
        <div>
          <label>Volume curve preset</label>
          <select value={v.volumeCurvePreset} onChange={(e) => upd("volumeCurvePreset", e.target.value)} disabled={disabled || busy}>
            {PRESETS.map((p) => (
        ...
        <div>
          <label>Custom weights (comma list, length == slices)</label>
          <input
            type="text"
            value={v.volumeCurveCustom}
            placeholder={customMode ? "e.g. 1,2,3,2,1" : "select 'Custom' to use"}
            onChange={(e) => upd("volumeCurveCustom", e.target.value)}
            disabled={disabled || busy || !customMode}
          />
        </div>
      </div>
      <div className="grid-2">
      ...
      <button type="submit" className="primary" disabled={disabled || busy}>{busy ? "Starting…" : "Start VWAP"}</button>
    </form>
  );
}
