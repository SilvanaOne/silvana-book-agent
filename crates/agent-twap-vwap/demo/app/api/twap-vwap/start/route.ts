...
import { buildVolumeCurve, type TwapConfig } from "@/lib/twap-engine";
...
function normalizeCurveSpec(v: unknown): string {
  if (v === undefined || v === null || v === "") return "u-shaped";
  if (Array.isArray(v)) {
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    const volumeCurveSpec = normalizeCurveSpec(body.volumeCurve);
    let volumeCurve: number[];
    try {
      volumeCurve = buildVolumeCurve(volumeCurveSpec, slicesNum);
    } catch (e) {
