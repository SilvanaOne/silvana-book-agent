...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/algo-order-dispatcher/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 500); return () => clearInterval(id); }, [refresh]);

  const start = async (v: FormValues) => {
    const r = await fetch("/api/algo-order-dispatcher/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ plan: v.plan, startingPrice: v.startingPrice, bookVolatility: v.bookVolatility }) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/algo-order-dispatcher/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/algo-order-dispatcher/reset", { method: "POST" }); refresh(); };
      ...
      {tab === "docs" && (
        <div className="card">
          ...
          <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
            <p><strong>agent-algo-order-dispatcher</strong> is a pluggable execution dispatcher. A plan lists any number of steps; each step names an algorithm and its parameters and the agent runs them sequentially against the orderbook. Three algos ship in-process: <span className="mono">twap</span> (equal time-sliced children), <span className="mono">iceberg</span> (small visible chunks, next placed only after previous clears), and <span className="mono">liquidity-seeking</span> (walk depth each cycle, size the child to the slippage budget).</p>
            <p>This demo executes a plan of up to 10 steps against a simulated book. Watch the progress bar per step fill in real-time and try nudging the mid outside an iceberg step&apos;s <span className="mono">price</span> — the iceberg stalls while later TWAP/liq-seek steps still deliver against the moving mid. The engine (<span className="mono">lib/algo-engine.ts</span>) mirrors <span className="mono">crates/agent-algo-order-dispatcher/src/main.rs</span> (<span className="mono">fn dispatcher</span> + per-algo runners).</p>
