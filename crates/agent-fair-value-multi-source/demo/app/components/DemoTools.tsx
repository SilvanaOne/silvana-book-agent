...
export function DemoTools({ fairvalue, onJump, onWalk }: Props) {
  const [manual, setManual] = useState("");
  const [drift, setDrift] = useState("0");
  const [vol, setVol] = useState("0.8");

  if (!fairvalue || fairvalue.status !== "monitoring") return <div className="muted">Start Fair Value to enable demo tools.</div>;

  const price = fairvalue.history.at(-1)?.truth ?? fairvalue.fairValue ?? fairvalue.config.startingPrice;
