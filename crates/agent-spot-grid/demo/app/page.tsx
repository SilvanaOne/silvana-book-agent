...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/spot-grid/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/spot-grid/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/spot-grid/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/spot-grid/reset", { method: "POST" }); refresh(); };
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              <p><strong>agent-spot-grid</strong> — minimal passive market-making grid on Canton. On start it stacks <span className="mono">bidLevels</span> BIDs below the mid and <span className="mono">offerLevels</span> OFFERs above, spaced by <span className="mono">stepPct</span>%, each with a fixed <span className="mono">qty/level</span>. As the mid walks through a rung, that order fills — and, unlike <span className="mono">agent-infinite-grid</span>, is NOT restored. No RFQ, no fill loops.</p>
              <p>The placement engine (<span className="mono">lib/spotgrid-engine.ts</span>) mirrors the Rust rules in <span className="mono">crates/agent-spot-grid/src/main.rs</span>. Prices are a GBM random walk — no real orders sent, and fills are simulated whenever the mid crosses a resting rung.</p>
