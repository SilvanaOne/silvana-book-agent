...
export function EventsLog({ events }: Props) {
  ...
  return (
    <div className="mono" style={{ fontSize: 12, lineHeight: 1.6, maxHeight: 260, overflow: "auto" }}>
      {items.map((e, i) => {
        const isAct = e.message.startsWith("ACT ");
        const isScan = e.message.startsWith("SCAN ");
        const color = isAct ? "#6de6a3" : isScan ? "#8a8a99" : "#b8c5df";
        return (
          <div key={i} style={{ display: "flex", gap: 10, borderBottom: "1px solid #1c1c24", padding: "4px 0" }}>
