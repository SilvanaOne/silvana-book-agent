...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      const r = await fetch("/api/killswitch-panic/state", { cache: "no-store" });
  ...
  const start = async (v: FormValues) => {
    ...
    const r = await fetch("/api/killswitch-panic/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const stop = async () => { await fetch("/api/killswitch-panic/stop", { method: "POST" }); refresh(); };
  const reset = async () => { await fetch("/api/killswitch-panic/reset", { method: "POST" }); refresh(); };
  ...
  const doPanic = async () => {
    setPanicBusy(true);
    try { await fetch("/api/killswitch-panic/panic", { method: "POST" }); await refresh(); }
              ...
              <div className="stack">
                ...
                <div className="card">
                  ...
                  <div style={{ fontSize: 13, color: "var(--text-faint)", marginBottom: 10, lineHeight: 1.6 }}>
                    Immediate cancel-all. Aborts the monitor loop and cancels every open order the agent has now. Corresponds to <span className="mono">agent-killswitch-panic panic</span>.
                  </div>
                  <button
                    onClick={doPanic}
        ...
        {tab === "docs" && (
          <div className="card">
            ...
            <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
              ...
              <p>The state machine (<span className="mono">lib/killswitch-engine.ts</span>) mirrors the Rust logic in <span className="mono">crates/agent-killswitch-panic/src/main.rs</span> (<span className="mono">run_monitor</span>, <span className="mono">evaluate</span>, <span className="mono">cancel_all</span>). No real orders are placed — this is a teaching model, not a backtest.</p>
