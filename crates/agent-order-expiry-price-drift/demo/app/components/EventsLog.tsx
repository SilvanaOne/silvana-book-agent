...
export function EventsLog({ events }: Props) {
  ...
  return (
    <div className="mono" style={{ fontSize: 12, lineHeight: 1.6, maxHeight: 260, overflow: "auto" }}>
      {items.map((e, i) => {
        const isCancel = e.message.startsWith("CANCELLED");
        const isDry = e.message.startsWith("[DRY-RUN]");
        const isNew = e.message.startsWith("NEW ORDER");
        const isSweep = e.message.startsWith("sweep");
        const color = isCancel ? "#ff8dab" : isDry ? "#ffd66b" : isNew ? "#6de6a3" : isSweep ? "#8ea1ce" : "#b8c5df";
        return (
