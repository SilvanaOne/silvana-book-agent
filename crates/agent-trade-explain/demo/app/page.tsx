...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try { const r = await fetch("/api/trade-explain/state", { cache: "no-store" }); const j = (await r.json()) as Snapshot; setSnap(j); } catch { /* ignore */ }
  }, []);
  useEffect(() => { refresh(); const id = setInterval(refresh, 1000); return () => clearInterval(id); }, [refresh]);

  const start = async (v: FormValues) => {
    const r = await fetch("/api/trade-explain/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(v) });
    if (!r.ok) { const j = (await r.json().catch(() => ({}))) as { error?: string }; throw new Error(j.error ?? `HTTP ${r.status}`); }
    refresh();
  };
  const reset = async () => { await fetch("/api/trade-explain/reset", { method: "POST" }); refresh(); };
  const rerun = async () => { await fetch("/api/trade-explain/rerun", { method: "POST" }); refresh(); };
      ...
      {tab === "docs" && (
        <div className="card">
          ...
          <div style={{ maxWidth: 720, lineHeight: 1.7, fontSize: 13.5 }}>
            <p><strong>agent-trade-explain</strong> reads a trading-history JSONL and generates, for every <span className="mono">order.created</span> or <span className="mono">settlement.settled</span>, a short rationale plus a counterfactual (&quot;what likely would have happened if we hadn&apos;t?&quot;). Output is a signed JSONL — auditors can attach it as decision-support alongside the trades themselves.</p>
          </div>
        </div>
      )}
    </div>
